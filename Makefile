# Makefile for the nocaselist project.
#
...
project_name := nocaselist

# Name of this Python package
package_name := nocaselist
...
help:
	...
	@echo "  test       - Run unit tests against local package"
...
.PHONY: all
all: install develop check_reqs build builddoc check ruff pylint mypy installtest test doclinkcheck authors
	@echo "Makefile: $@ done."
...
.PHONY: testlist
testlist: $(test_unit_py_files)
	@echo "Makefile: Running unit tests against standard list"
	TEST_AGAINST_LIST=1 py.test --color=yes $(pytest_warning_opts) $(pytest_opts) $(test_unit_dir) -s
